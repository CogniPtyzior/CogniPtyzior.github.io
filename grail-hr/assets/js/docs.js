...
(function(){
  ...
  document.querySelectorAll('pre').forEach(pre => {
    ...
    const btn = document.createElement('button'); btn.className = 'copy-btn'; btn.type = 'button'; btn.textContent = 'Copier';
    btn.addEventListener('click', async () => {
      try { await navigator.clipboard.writeText(pre.innerText); btn.textContent = 'Copié'; setTimeout(() => btn.textContent = 'Copier', 1300); } catch(e) {}
    });
